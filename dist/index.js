...
var EXTENDED_UNDERLINE_TERMS = ["xterm-ghostty", "xterm-kitty", "wezterm", "xterm-256color"];
var EXTENDED_UNDERLINE_PROGRAMS = ["Ghostty", "iTerm.app", "WezTerm"];
function detectExtendedUnderline() {
  const term = process.env.TERM ?? "";
  const termProgram = process.env.TERM_PROGRAM ?? "";
...
// src/term.ts
var ANSI_REGEX = /\x1b\[[0-9;:]*m|\x1b\]8;;[^\x1b]*\x1b\\/g;
...
var STDERR_METHODS = new Set(["error", "warn"]);
...
// src/utils.ts
var ANSI_REGEX2 = /\x1b\[[0-9;:]*m|\x1b\]8;;[^\x1b]*\x1b\\/g;
function stripAnsi3(text) {
  return text.replace(ANSI_REGEX2, "");
}
function displayLength(text) {
  return stringWidth(stripAnsi3(text));
}
// src/underline.ts
...
function styledUnderline(style, rgb, text) {
  if (!supportsExtendedUnderline()) {
    return chalk2.underline(text);
  }
  const [r, g, b] = rgb;
...
var BG_OVERRIDE_CODE = 9999;
function bgOverride(text) {
...
var src_default = chalkX;
export {
  underlineColor,
  underline,
  supportsExtendedUnderline,
  styledUnderline,
  stripAnsi3 as stripAnsi,
